/**
 * Generates ML training results for FocusMeet engagement model.
 * Pure Node.js — no external dependencies. Runs the exact same math
 * as the Python pipeline would.
 */
import { writeFileSync, mkdirSync, existsSync } from 'fs';
import { join, dirname } from 'path';
import { fileURLToPath } from 'url';
const __dirname = dirname(fileURLToPath(import.meta.url));
const ML = join(__dirname,'..','backend','ml');
if(!existsSync(ML))mkdirSync(ML,{recursive:true});

const F=['focus_score','face_detected','gaze_variance','blink_rate','mic_active_pct','speaking_turns','words_per_min','typing_events_per_min','chat_messages_in_window','chat_sentiment_avg','reaction_count_in_window','poll_participation'];
const N=2000,T=0.2,p=F.length;
let _s=42;const sr=s=>{_s=s};const rn=()=>{_s=(_s*16807)%2147483647;return(_s-1)/2147483646};
...
const ch=(it,wt)=>{const t=wt.reduce((a,b)=>a+b);let r=rn()*t,a=0;for(let i=0;i<it.length;i++){a+=wt[i];if(r<=a)return it[i]}return it.at(-1)};

function gen(){const ar=ch(['F','P','D','A','M'],[35,25,15,15,10]);let f,fc,gv,br,mi,tu,wp,ty,cn,se,re,po,en;
...
const r2s=(yt,yp)=>{const m=yt.reduce((a,b)=>a+b)/yt.length;const sr=yt.reduce((s,v,i)=>s+(v-yp[i])**2,0);const st=yt.reduce((s,v)=>s+(v-m)**2,0);return st>0?1-sr/st:0};
function v1(row){const fo=row.focus_score,fa=!!row.face_detected,mi=row.mic_active_pct,w=row.words_per_min,t=row.typing_events_per_min;
...
sr(42);const rows=Array.from({length:N},gen);
const means=F.map(f=>rows.reduce((s,r)=>s+r[f],0)/N);
const stds=F.map((f,j)=>{const m=means[j];return Math.sqrt(rows.reduce((s,r)=>s+(r[f]-m)**2,0)/N)||1});
const XA=rows.map(r=>[1,...F.map((f,j)=>(r[f]-means[j])/stds[j])]);
const yA=rows.map(r=>r.self_reported_score);
const idx=Array.from({length:N},(_,i)=>i);
for(let i=idx.length-1;i>0;i--){const j=Math.floor(rn()*(i+1));[idx[i],idx[j]]=[idx[j],idx[i]]}
const sp=Math.floor(N*(1-T)),trI=idx.slice(0,sp),teI=idx.slice(sp);
const XTr=trI.map(i=>XA[i]),yTr=trI.map(i=>yA[i]),XTe=teI.map(i=>XA[i]),yTe=teI.map(i=>yA[i]);

const mods={};
for(const[nm,l]of[['LinearRegression',0],['Ridge (α=1.0)',1],['Ridge (α=10)',10]]){
const w=fit(XTr,yTr,l),yp=pred(XTe,w),m=mae(yTe,yp),rv=r2s(yTe,yp);
mods[nm]={w,mae:+m.toFixed(3),r2:+rv.toFixed(4)};
console.log(`  ${nm}: MAE=${m.toFixed(3)} R²=${rv.toFixed(4)}`)}
const bn=Object.keys(mods).reduce((a,b)=>mods[a].r2>mods[b].r2?a:b);
const best=mods[bn];
console.log(`\n  ★ Best: ${bn} (MAE=${best.mae}, R²=${best.r2})`);

const ca=F.map((_,j)=>Math.abs(best.w[j+1]));
const ti=ca.reduce((a,b)=>a+b)||1;
const imp={};F.forEach((f,j)=>{imp[f]=+(ca[j]/ti*100).toFixed(2)});
const si=Object.entries(imp).sort((a,b)=>b[1]-a[1]);
console.log('\n  Feature importances:');
for(const[f,v]of si)console.log(`    ${f.padEnd(30)} ${v.toFixed(2)}%`);

const yV1=teI.map(i=>v1(rows[i])),mV1=mae(yTe,yV1),rV1=r2s(yTe,yV1);
console.log(`\n  v1 fallback: MAE=${mV1.toFixed(3)} R²=${rV1.toFixed(4)}`);
const mi2=(mV1-best.mae)/mV1*100,ri2=best.r2-rV1;
console.log(`  Improvement: MAE -${mi2.toFixed(1)}%, R² ${ri2>=0?'+':''}${ri2.toFixed(4)}`);

const out={dataset_size:N,test_size:teI.length,feature_count:p,feature_order:F,
models:Object.fromEntries(Object.entries(mods).map(([k,v])=>[k,{mae:v.mae,r2:v.r2}])),
...
writeFileSync(join(ML,'ml_results.json'),JSON.stringify(out,null,2));
writeFileSync(join(ML,'fusion_model.json'),JSON.stringify({model_type:bn,weights:best.w,feature_order:F,means,stds,bias:best.w[0]},null,2));
console.log(`\n  ✓ Saved results and model to backend/ml/`);
